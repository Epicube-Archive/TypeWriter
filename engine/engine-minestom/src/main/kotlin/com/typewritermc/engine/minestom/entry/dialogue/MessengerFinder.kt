...
import com.typewritermc.engine.minestom.adapt.event.bukkit.PlayerJumpEvent
...
import com.typewritermc.engine.minestom.utils.reloadable
...
enum class ConfirmationKey(val keybind: String) {
    ...
    private inline fun <reified E : PlayerEvent> listenEvent(
        listener: com.typewritermc.engine.minestom.adapt.event.Listener,
        playerUUID: UUID,
    ...
    fun listen(listener: com.typewritermc.engine.minestom.adapt.event.Listener, playerUUID: UUID, block: () -> Unit) {
        when (this) {
