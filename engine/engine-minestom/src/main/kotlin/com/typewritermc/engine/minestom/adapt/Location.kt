...
class Location(val instance: Instance?, val position: Pos): Point, Rotatable {
    ...
    override fun withY(y: Double): Location = copy(y = y)

    override fun withZ(z: Double): Location = copy(z = z)

    override fun withYaw(yaw: Float): Location = copy(yaw = yaw)

    override fun withPitch(pitch: Float): Location = copy(pitch = pitch)

    override fun rotateYaw(angle: Float): Location = copy(yaw = this.yaw + angle)

    override fun rotatePitch(angle: Float): Location = copy(pitch = this.pitch + angle)

    override fun rotate(yaw: Float, pitch: Float): Location = copy(yaw = this.yaw + yaw, pitch = this.pitch + pitch)
    ...
    override fun sub(x: Double, y: Double, z: Double): Location {
        return copy(x = this.x - x, y = this.y - y, z = this.z - z)
    }
    ...
    override fun mul(x: Double, y: Double, z: Double): Location {
        return copy(x = this.x * x, y = this.y * y, z = this.z * z)
    }
    ...
    override fun div(x: Double, y: Double, z: Double): Location {
        return copy(x = this.x / x, y = this.y / y, z = this.z / z)
    }
    ...
    override fun lookAt(point: Point): Location {
    ...
    override fun resetRotation(): Location = copy(yaw = 0f, pitch = 0f)
    ...
    fun isInRange(point: Location, range: Double): Boolean {
    ...
    open fun copy(
        ...
        return Location(world, Pos(x, y, z, yaw, pitch))
    }
